...
export function PesquisarItem() {
  const navigate = useNavigate();
  const [itens, setItens] = useState<ItemAlerta[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);
  const [filtros, setFiltros] = useState<FiltrosItem>({ todos: false });
  ...
  const carregarItens = useCallback(async () => {
    ...
    setError(false);
    try {
      const data = await itensApi.porStatus(filtros);
      setItens(data);
    } catch (err) {
  ...
  // Carrega itens ao montar e quando filtros mudam
  useEffect(() => {
    carregarItens();
  }, [carregarItens]);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="sticky top-16 z-40 bg-gray-50 pt-4 pb-2 px-4 -mx-4 shadow-sm">
        {/* Header */}
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Pesquisar por Item</h1>
            <p className="text-gray-500 text-sm">Busca inteligente de itens em atas</p>
          </div>
        </div>
        ...
        <Card>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Buscar por descrição (ex: dipirona 500mg)..."
                value={buscaInput}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-gray-200">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFiltros(prev => ({ ...prev, tipoItem: e.target.value || undefined }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#00508C] focus:border-transparent"
                >
              ...
              <div className="sm:col-span-2 lg:col-span-4 flex items-center justify-between pt-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-gray-600">
                    Incluir todos (sem limite de 30 dias vencidos)
                  </span>
                </label>
                {temFiltrosAtivos && (
                  <button
      ...
      <div className="p-4 space-y-6">
        {loading ? (
          ...
          <EmptyState
            ...
            description={temFiltrosAtivos ? "Tente ajustar os filtros" : "Não há itens com ata"}
          />
        ) : (
          <Card>
          <CardHeader>
